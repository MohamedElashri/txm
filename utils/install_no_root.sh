...
echo -e "${BLUE}Moving the txm binary to $BIN_DIR...${NC}"
mv "txm-$OS" "$BIN_DIR/txm" || error_exit "Failed to move the txm binary to $BIN_DIR."
...
# Move the man page to the user's man directory
echo -e "${BLUE}Moving the txm man page to $MAN_DIR...${NC}"
mv "txm.1" "$MAN_DIR/txm.1" || error_exit "Failed to move the txm man page to $MAN_DIR."

# Update the man page index
echo -e "${BLUE}Updating the man page index...${NC}"
if [ "$OS" == "macOS" ]; then
  /usr/libexec/makewhatis "$MAN_DIR"
else
  mandb "$MAN_DIR"
fi
